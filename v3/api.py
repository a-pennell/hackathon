...
def sign_visit(pid: str, body: VisitSignBody, as_of: str | None = None):
    ...
    if stem:
        accept, reject_yes, reject_unconfirmed = [], [], []
        for p in v["proposals"]:
            if p["status"] != "proposed" or p.get("change"):
                continue
            ids = list(dict.fromkeys([p["id"], *p.get("link_ids", [])]))      # the thing and the links it makes
            cause_ids = p.get("cause_link_ids") or ([p["id"]] if p["id"].startswith("lnk_") else [])
            rest = [i for i in ids if i not in cause_ids]
            if p["origin"] == "stated":
                accept += ids
            elif body.decisions.get(p["id"]) == "accept":
                accept += ids
            elif body.decisions.get(p["id"]) == "reject":
                reject_yes += cause_ids; accept += rest                        # the inferred relation goes; the thing itself stays
            else:
                reject_unconfirmed += cause_ids; accept += rest
        accept = list(dict.fromkeys(accept))
        reason_of = {}
        for p in v["proposals"]:
            for cid in p.get("cause_link_ids") or [p["id"]]:
