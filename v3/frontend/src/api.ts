...
import type { Commitments, Draft, Listing, ManifestGroup, NoteDoc, NoteView, ProblemView, VisitData } from "./v2types";
...
export const api = {
  ...
  signVisit: (pid: string, body: { decisions: Record<string, string>; reasons: Record<string, string>; authored: string; sections?: { key: string; text: string }[] }) => req<{ document_id: string; attested: number }>(`/v3/api/patients/${pid}/visit/sign${q()}`, post(body)),
  commitments: (pid: string) => req<Commitments>(`/v3/api/patients/${pid}/commitments${q()}`),
  previewVisit: (pid: string, body: { decisions: Record<string, string>; reasons: Record<string, string>; authored: string; sections?: { key: string; text: string }[]; heard?: string[] | null; upto?: number | null }) => req<Draft>(`/v3/api/patients/${pid}/visit/preview${q()}`, post(body)),
