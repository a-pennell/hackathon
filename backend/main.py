...
from ehr.draft import run_draft, sign_draft, stem_for  # noqa: E402
...
def get_draft(pid: str, eid: str):
    """The queued visit note for an encounter, if one has been compiled."""
...
def compile_draft(pid: str, eid: str):
    """Compile the visit note from the encounter's decisions. Deterministic; nothing reaches the chart until signed."""
    try:
        return run_draft(pid, eid, data_dir=DATA_DIR)
    except KeyError as e:
        raise HTTPException(404, str(e))
