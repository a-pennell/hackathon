...
export function nextAction(overview: Overview | null, notes: NoteFile[], queues: QueueBatch[], summary: PatientSummary | null): NextAction {
  ...
  if (here) {
    ...
    if (waiting > 0) return { label: `Review the note · ${waiting}`, hint: "decide the few things that need a decision, then sign it", kind: "review", noteId: here.id };
  ...
  for (const c of concerns) {
    ...
    if (pendingInsights > 0) return { label: `Sign ${pendingInsights} insight${pendingInsights > 1 ? "s" : ""} on ${short}`, hint: "each signed insight becomes a sentence in the visit note", kind: "sign-insights", problemId: c.id };
