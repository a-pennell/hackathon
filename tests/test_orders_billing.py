...
def signed(by="Dr. Chen"):
    """A signature stamped on the fixture's day, so the visit coding is not a function of the wall clock."""
    return {**review_record("accepted", by), "at": f"{TODAY}T09:30:00-04:00"}
...
def test_orders_validate(patient):
    ...
    done = accept_item(chart, batch, "ord_ckd_01", review=signed())
