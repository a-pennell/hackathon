...
def test_signed_item_requires_correction_and_preserves_original_history(patient, note, raw, tmp_path):
    from ehr.review import apply_review, undo_review
    from ehr.corrections import apply_correction, preview
    (tmp_path / "patients").mkdir(); (tmp_path / "proposed" / "pt_t").mkdir(parents=True)
    ...
    with pytest.raises(ValueError, match="recorded correction"):
        undo_review("pt_t", "note_demo_002", [nap], data_dir=tmp_path / "patients")
    impact = preview("pt_t", nap, "entered_in_error", tmp_path / "patients")
    apply_correction("pt_t", nap, "entered_in_error", reason="Wrong medication", expected_revision=impact["revision"], data_dir=tmp_path / "patients")
    chart = json.loads((tmp_path / "patients" / "pt_t.json").read_text())
    ...
    assert next(m for m in q["proposed"]["medications"] if m["id"] == nap)["status"] == "entered_in_error"
    assert next(l for l in q["proposed"]["links"] if l["id"] == treats["id"])["status"] == "superseded"
    assert any(x["item"]["id"] == nap for x in chart["archived_items"])
